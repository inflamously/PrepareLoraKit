...
// The default selection: every non-optional step, whatever its run state.
export function selectSteps() {
    if (!state.project) return;

    applySelection(defaultActiveSteps());
}

// Only what the pipeline still has work for, optional steps included.
export function selectPending() {
    if (!state.project) return;

    applySelection(unfinishedSteps());
}
...
function applySelection(stepTypes) {
    state.selectedSteps = stepTypes;
    state.selectedSubsteps = selectedAvailableSubsteps(stepTypes);
    render();
}
...
// `skipped` and `stale` count as unfinished: the first ran but did no work, the
// second lost the report backing its claim, so both still want a run. Optional
// steps stay out regardless of status — they are opt-in, so "still to do" is not
// something the pipeline decides for them.
