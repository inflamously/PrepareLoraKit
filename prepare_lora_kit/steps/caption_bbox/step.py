"""
Step 5 — Caption with Bbox Annotation + Hugging Face captioning

For each image:
  1. Optional region annotations are collected (UI-only; the CLI captions full images).
  2. Bbox context + image are sent to a HF caption model to produce a structured caption.
  3. Caption is cleaned, token-checked, and saved as {stem}.txt.
"""
from __future__ import annotations
from pathlib import Path
from typing import Any, Callable

from prepare_lora_kit.steps.caption_bbox.regions import make_region_captioner
from prepare_lora_kit.cancellation import CancelCheck, check_cancel
from prepare_lora_kit.providers.interaction import InteractionProvider
from prepare_lora_kit.utils import image as img_utils
from prepare_lora_kit.report import reporter

import prepare_lora_kit.steps.caption_bbox.vlm as vlm
from prepare_lora_kit.steps.caption_bbox.artifacts import (
    BBOX_PREFIX,
    _bbox_stem,
    _clean_bbox_artifacts,
    _is_bbox_artifact,
    _save_bbox_training_item,
    save_boxes_sidecar,
)
from prepare_lora_kit.steps.caption_bbox.reports import _save_failure_report, build_success_report, save_success_report
from prepare_lora_kit.steps.caption_bbox.validation import render_spot_check, validate_captions
from prepare_lora_kit.steps.caption_bbox.workflow import CaptionWorkflowResult, gather_decisions, resolve_decision, \
    _persist_region_caption_edits, _caption_full_image, _write_caption

DEFAULT_SUBSTEPS = ["annotate_regions", "caption_images", "validate_captions"]
__all__ = [
    "run",
    "BBOX_PREFIX",
    "_bbox_stem",
    "_clean_bbox_artifacts",
    "_is_bbox_artifact",
    "_save_bbox_training_item",
    "_save_failure_report",
]


# ── Main ──────────────────────────────────────────────────────────────────────

def run(
        dataset_dir: Path,
        concept_token: str | None = None,
        output_dir: Path | None = None,
        caption_model_id: str | None = None,
        caption_model_task: str = "auto",
        spot_check_pct: float = 0.10,
        overwrite: bool = False,
        report_path: Path | None = None,
        quantization: str = "none",
        dtype: str = "bfloat16",
        max_new_tokens: int = 200,
        max_pixels: int = vlm._DEFAULT_MAX_PIXELS,
        interaction: InteractionProvider | None = None,
        enabled_substeps: list[str] | None = None,
        cancel_check: CancelCheck | None = None,
        caption_status_callback: Callable[[dict[str, Any]], None] | None = None,
        qwen_model_id: str | None = None,
        caption_prompt: str | None = None,
        region_prompt: str | None = None,
) -> dict:
    style_mode = not concept_token
    reporter.step_header("Caption — Bbox Annotation + HF Captioning")
    enabled = set(enabled_substeps or DEFAULT_SUBSTEPS)
    model_id = str(caption_model_id or qwen_model_id or "").strip()

    output_dir = _prepare_output_dir(output_dir or dataset_dir)
    all_images, images = _collect_source_images(dataset_dir)
    if not all_images:
        reporter.warn(f"No images in {dataset_dir}")
        return {}

    _log_caption_mode(images, concept_token, style_mode=style_mode)

    check_cancel(cancel_check)
    img_utils.materialize(all_images, dataset_dir, output_dir)

    # Resume: only images that still lack a caption need work (``overwrite`` — set by
    # --force — recaptions everything). When nothing is pending we skip loading the
    # VLM entirely and just rebuild the report from the existing on-disk captions.
    txt_paths, pending = _resolve_pending(images, dataset_dir, output_dir, overwrite)
    needs_captioning = bool(pending) and "caption_images" in enabled
    if needs_captioning:
        _require_model_for_captioning(enabled, model_id)

    runtime = vlm.CaptionRuntime(
        model_id,
        task=caption_model_task,
        quantization=quantization,
        dtype=dtype,
        max_pixels=max_pixels,
        status_callback=caption_status_callback,
        caption_prompt=caption_prompt,
        region_prompt=region_prompt,
    )

    try:
        # Load caption model only when there is captioning work to do.
        if needs_captioning:
            runtime.load()

        caption_result = _caption_dataset(
            images,
            pending=pending,
            txt_paths=txt_paths,
            output_dir=output_dir,
            overwrite=overwrite,
            enabled=enabled,
            interaction=interaction,
            runtime=runtime,
            concept_token=concept_token,
            style_mode=style_mode,
            max_new_tokens=max_new_tokens,
            report_path=report_path,
            cancel_check=cancel_check,
        )
        return _validate_and_save_success(
            images,
            caption_result,
            runtime=runtime,
            concept_token=concept_token,
            style_mode=style_mode,
            enabled=enabled,
            spot_check_pct=spot_check_pct,
            report_path=report_path,
            output_dir=output_dir,
            cancel_check=cancel_check,
        )
    finally:
        runtime.unload()


def _caption_dataset(
        images: list[Path],
        *,
        pending: list[Path],
        txt_paths: dict[Path, Path],
        output_dir: Path,
        overwrite: bool,
        enabled: set[str],
        interaction: InteractionProvider | None,
        runtime: vlm.CaptionRuntime,
        concept_token: str | None,
        style_mode: bool,
        max_new_tokens: int,
        report_path: Path | None,
        cancel_check: CancelCheck | None,
) -> CaptionWorkflowResult:
    result = CaptionWorkflowResult()
    region_captioner = make_region_captioner(
        runtime=runtime,
        output_dir=output_dir,
        captions=result.captions,
        concept_token=concept_token,
        cancel_check=cancel_check,
    )

    # Resume: leave already-captioned images (and their hand-drawn boxes) untouched,
    # but load their captions into the report so it stays complete. Only ``pending``
    # images are surfaced for annotation and (re)captioned below.
    pending_set = set(pending)
    for path in images:
        if path in pending_set:
            continue
        txt_path = txt_paths[path]
        if txt_path.exists():
            result.captions[str(path)] = txt_path.read_text(encoding="utf-8").strip()

    # Phase A — gather bbox annotations for the pending images only, in one workspace
    # interaction. Reloaded boxes (from a prior run) are handed to the UI so a forced
    # re-caption keeps them prefilled. Skip the interaction entirely when nothing is
    # pending so a resume never pops an empty annotation modal.
    decisions = gather_decisions(
        pending,
        txt_paths=txt_paths,
        overwrite=overwrite,
        enabled=enabled,
        provider=interaction,
        region_captioner=region_captioner,
        result=result,
        cancel_check=cancel_check,
    ) if pending else {}

    # Phase B — caption each pending, non-skipped image with the submitted annotations.
    for path in pending:
        check_cancel(cancel_check)
        txt_path = txt_paths[path]
        txt_path.parent.mkdir(parents=True, exist_ok=True)

        annotations = resolve_decision(
            path,
            txt_path,
            decisions.get(str(path)),
            overwrite=overwrite,
            enabled=enabled,
            result=result,
        )
        if annotations is None:
            continue

        _persist_region_caption_edits(
            annotations,
            result.captions,
            concept_token=concept_token,
        )
        save_boxes_sidecar(path, annotations)
        caption = _caption_full_image(
            path,
            annotations,
            images=images,
            enabled=enabled,
            result=result,
            runtime=runtime,
            concept_token=concept_token,
            max_new_tokens=max_new_tokens,
            report_path=report_path or (output_dir / "step5_report.json"),
            cancel_check=cancel_check,
        )
        _write_caption(
            path,
            txt_path,
            caption,
            result.captions,
            concept_token=concept_token,
            style_mode=style_mode,
            cancel_check=cancel_check,
        )

    return result


def _validate_and_save_success(
        images: list[Path],
        caption_result: CaptionWorkflowResult,
        *,
        runtime: vlm.CaptionRuntime,
        concept_token: str | None,
        style_mode: bool,
        enabled: set[str],
        spot_check_pct: float,
        report_path: Path | None,
        output_dir: Path,
        cancel_check: CancelCheck | None,
) -> dict:
    check_cancel(cancel_check)
    missing_token, short, long_ = validate_captions(
        caption_result.captions,
        concept_token,
        style_mode=style_mode,
        enabled=enabled,
        cancel_check=cancel_check,
    )
    sample = render_spot_check(
        caption_result.captions,
        spot_check_pct,
        enabled=enabled,
        cancel_check=cancel_check,
    )

    report_data = build_success_report(
        images=images,
        captions=caption_result.captions,
        runtime=runtime,
        skipped_annotation=caption_result.skipped_annotation,
        missing_token=missing_token,
        short_captions=short,
        long_captions=long_,
        spot_check_sample=sample,
        enabled=enabled,
    )
    check_cancel(cancel_check)
    save_success_report(report_data, report_path, output_dir)
    return report_data


def _prepare_output_dir(output_dir: Path) -> Path:
    # Never wipe bbox artifacts here: the ``plk_bbox__*__boxes.json`` reload sidecars
    # store the user's hand-drawn boxes and must survive every re-run (including
    # --force). Captions are regenerated by overwriting their .txt on write, so no
    # pre-clean is needed. ``_clean_bbox_artifacts`` remains for an explicit
    # "clear boxes" action.
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir


def _resolve_pending(
        images: list[Path],
        dataset_dir: Path,
        output_dir: Path,
        overwrite: bool,
) -> tuple[dict[Path, Path], list[Path]]:
    """Map each image to its caption ``.txt`` and list which images still need one.

    ``pending`` is every image when ``overwrite`` (a --force re-caption); otherwise
    only images whose caption file does not yet exist — the resume set.
    """
    txt_paths = {
        path: (output_dir / path.relative_to(dataset_dir)).with_suffix(".txt")
        for path in images
    }
    if overwrite:
        pending = list(images)
    else:
        pending = [path for path in images if not txt_paths[path].exists()]
    return txt_paths, pending


def _collect_source_images(dataset_dir: Path) -> tuple[list[Path], list[Path]]:
    all_images = img_utils.iter_images(dataset_dir)
    images = [p for p in all_images if not _is_bbox_artifact(p)]
    return all_images, images


def _log_caption_mode(
        images: list[Path],
        concept_token: str | None,
        *,
        style_mode: bool,
) -> None:
    if style_mode:
        reporter.info(f"Captioning {len(images)} images in style mode (no concept token).")
    else:
        reporter.info(f"Captioning {len(images)} images. Concept token: '{concept_token}'")


def _require_model_for_captioning(enabled: set[str], model_id: str) -> None:
    if "caption_images" in enabled and not model_id:
        raise RuntimeError("CaptionBboxStep requires caption_model_id before captioning can run.")
